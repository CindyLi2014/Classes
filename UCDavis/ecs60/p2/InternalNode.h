...
public:
  ...
  InternalNode* split(BTreeNode *last);
	...
	//Cindy Li
	BTreeNode* removeChild(int pos);
private:
	InternalNode* _borrowValFromLeftSibling();
	InternalNode* _mergeWithLeftSibling();
	InternalNode* _borrowValFromRightSibling(int pos);
	InternalNode* _mergeWithRightSibling();
